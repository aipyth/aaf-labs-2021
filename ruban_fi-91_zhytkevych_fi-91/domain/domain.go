...
func (d *Domain) InsertDocument(collectionName string, document string) error {
    ...
    err = d.Indexer.IndexDocument(int(doc.Id), []byte(document))
    if err != nil {
        return err
    }
...
func (d *Domain) Search(q SearchQuery) []*storage.Document {
    ...
    case q.KeywordE != "" && q.Keyword != "":
        idsPos := d.Indexer.GetDocsByKeywords(q.Keyword, q.KeywordE, q.N)
        for _, v := range idsPos {
    ...
    case q.Prefix != "":
        idsPos := d.Indexer.GetDocsByPrefix(q.Prefix)
        for _, v := range idsPos {
            doc, err := d.CollectionStorage.GetDocumentById(v)
            if err == nil {
                resp = append(resp, doc)
    ...
    case q.Keyword != "":
        idsPos := d.Indexer.GetDocsByKeyword(q.Keyword)
        for _, v := range idsPos {
            doc, err := d.CollectionStorage.GetDocumentById(v)
            if err == nil {
                resp = append(resp, doc)
